...
void ekfNav::ekf_init(uint64_t time/*, unsigned long TOW*/,   /* Time, Time of the week from GPS */
...
//initial gyro value for bias
gbx = p;
gby = q;
gbz = r;
std::tie(theta,phi,psi) = getPitchRollYaw(ax,ay,az,hx,hy,hz);
//theta for yaw, phi for pitch, psi  for roll
quat = toQuaternion(theta,phi,psi);
// gravity for z is G
grav(2,0) = G;
...
void ekfNav::ekf_update( uint64_t time/*, unsigned long TOW*/, double vn,double ve,double vd,double lat,double lon,double alt,
...
void ekfNav::updateCalculatedVsPredicted() {
...
void ekfNav::update15statesAfterKF() {
      estimated_ins = llarate ((x.block(0,0,3,1)).cast<double>(), lat_ins, alt_ins);
      lat_ins += estimated_ins(0,0);
      lon_ins += estimated_ins(1,0);
      alt_ins += estimated_ins(2,0);
      vn_ins = vn_ins + x(3,0);
      ve_ins = ve_ins + x(4,0);
      vd_ins = vd_ins + x(5,0);
      // Attitude correction
 ...
 void ekfNav::updateBias(float ax,float ay,float az,float p,float q,float r){
  f_b(0,0) = ax - abx;
  f_b(1,0) = ay - aby;
  f_b(2,0) = az - abz;
  om_ib(0,0) = p - gbx;
 ...
 std::tuple<float,float,float> ekfNav::getPitchRollYaw(float ax, float ay, float az, float hx, float hy, float hz){
...
Eigen::Matrix<float,4,1> ekfNav::toQuaternion(float yaw, float pitch, float roll) {
...
Eigen::Matrix<float,4,1> ekfNav::qmult(Eigen::Matrix<float,4,1> p, Eigen::Matrix<float,4,1> q) {
...
// quaternion to dcm
Eigen::Matrix<float,3,3> ekfNav::quat2dcm(Eigen::Matrix<float,4,1> q) {
...
std::tuple<float, float, float> ekfNav::toEulerAngles(Eigen::Matrix<float,4,1> quat) {
...
Eigen::Matrix<double,3,1> ekfNav::llarate(Eigen::Matrix<double,3,1> V,Eigen::Matrix<double,3,1> lla) {
...
// This function calculates the rate of change of latitude, longitude, and altitude.
Eigen::Matrix<double,3,1> ekfNav::llarate(Eigen::Matrix<double,3,1> V, double lat, double alt) {
...
void ekfNav::updateJacobianMatrix() {
    // Jacobian
  Fs.setZero();
  // ... pos2gs
...
Eigen::Matrix<float,3,3> ekfNav::sk(Eigen::Matrix<float,3,1> w) {
...
constexpr std::pair<double, double> ekfNav::earthradius(double lat) {
...
void ekfNav::updateProcessNoiseCovarianceTime(float _dt) {
...
Eigen::Matrix<double,3,1> ekfNav::lla2ecef(Eigen::Matrix<double,3,1> lla) {
  double Rew, denom;
...
Eigen::Matrix<double,3,1> ekfNav::ecef2ned(Eigen::Matrix<double,3,1> ecef,Eigen::Matrix<double,3,1> pos_ref) {
