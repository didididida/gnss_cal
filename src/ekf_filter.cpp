...
GpsImuVelFilter::GpsImuVelFilter(ros::NodeHandle &nh){
    ...
    //the transformation of enu to ned, for ekf here using ned
    ...
    ekfNav_ptr_ = std::make_unique<ekfNav>();

    // Initialize error charasteristics
    ekfNav_ptr_->setAcclNoise(accl_noise);
    ekfNav_ptr_->setAcclBias(accl_bias_noise);
    ekfNav_ptr_->setGyroNoise(gyro_noise);
    ekfNav_ptr_->setGyroBias(gyro_bias_noise);
    ekfNav_ptr_->setStdDevGpsPosNE(sigma_gps_pos_ne);
    ekfNav_ptr_->setStdDevGpsPosD(sigma_gps_pos_d);
    ekfNav_ptr_->setStdDevGpsVelNE(sigma_gps_vel_ne);
    ekfNav_ptr_->setStdDevGpsVelD(sigma_gps_vel_d);
...
void GpsImuVelFilter::MagCallback(const sensor_msgs::MagneticFieldPtr& mag_msg_ptr)
{
    ...
    ekfNav_ptr_->magDataUpdateEKF(mag_data_ptr);
}
...
void GpsImuVelFilter::ImuCallback(const sensor_msgs::ImuConstPtr& imu_msg_ptr){
    ...
    if(ekfNav_ptr_->imuDataUpdateEKF(imu_data_ptr,&fused_state)!=true){
        return;
    }
    // Publish fused state.
...
void GpsImuVelFilter::GpsPosCallback(const sensor_msgs::NavSatFixConstPtr& gps_msg_ptr){
    ...
    gps_data_ptr->pose_time = gps_msg_ptr->header.stamp.toSec();
